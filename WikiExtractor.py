...
"""Wikipedia Extractor:
Extracts and cleans text from Wikipedia database dump and stores each
article in file named as article title in AURORA code. Taken out from
Tanl software package.
    ...
    def __clean(self, wiki_document):
        ...
        for pattern, placeholder in self.__placeholder_tag_patterns:
            ...
            for match in pattern.finditer(wiki_document.text):
                wiki_document.text = wiki_document.text.replace(match.group(), '[%s %d]' % (placeholder, index))
                index += 1
        ...
        for match in good_wikilink_pattern.finditer(wiki_document.text):
            ...
            wiki_document.text = wiki_document.text.replace(wikilink, self.__handle_wikilink(wikilink[2:-2]))
        for match in good_wikilink_pattern.finditer(wiki_document.text):
            wikilink = match.group()
            wiki_document.text = wiki_document.text.replace(wikilink, self.__handle_wikilink(wikilink[2:-2]))
        ...
        for match in bad_left_wikilink_pattern.finditer(wiki_document.text):
            wikilink = match.group()
            wiki_document.text = wiki_document.text.replace(wikilink, self.__handle_wikilink(wikilink[1:-2]))
        bad_right_wikilink_pattern = self.__wikilink_pattern[2]
        for match in bad_right_wikilink_pattern.finditer(wiki_document.text):
            wikilink = match.group()
            wiki_document.text = wiki_document.text.replace(wikilink, self.__handle_wikilink(wikilink[2:-1]))
        wiki_document.text = wiki_document.text.replace('[[', '').replace(']]', '')
...
def process_data(input_file, wiki_extractor, output_dir_name):
    page = []
    for line in input_file:
        ...
        if line == '<page>':
            ...
            process_page(page, wiki_extractor, output_dir_name)
        else:
            page.append(line)
...
# Unicode --> AURORA
__trans = {u'а':'a', u'б':'b', u'в':'v', u'г':'g', u'д':'d',
           u'ђ':'dx', u'е':'e', u'ж':'zx', u'з':'z', u'и':'i',
           u'ј':'j', u'к':'k', u'л':'l', u'љ':'lx', u'м':'m',
           u'н':'n', u'њ':'nx', u'о':'o', u'п':'p', u'р':'r',
           u'с':'s', u'т':'t', u'ћ':'cx', u'ч':'cy', u'ф':'f',
           u'х':'h', u'ц':'c', u'у':'u', u'џ':'dy', u'ш':'sx',
...
def translate_article_name(name):
    newname = ''
    name = name.replace(' ','_')
    for n in name:
        if n in __trans:
...
def process_page(page, wiki_extractor, output_dir_name):
    ...
    title = translate_article_name(wiki_document.title) 
    if not title:
	title = "%d_Unknown" % num_of_unknown
	num_of_unknown += 1

    f = open(os.path.join(output_dir_name, "%s.txt" % title), 'w+')
    f.write(wiki_document.__str__().encode('utf-8'))
    f.close()
...
def main():
    ...
    process_data(sys.stdin, wiki_extractor, output_dir_name)
